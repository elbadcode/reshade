...
reshade::runtime::runtime(api::swapchain *swapchain, api::command_queue *graphics_queue, const std::filesystem::path &config_path, bool is_vr) :
    ...
    _screenshot_path(L".\\"),
    _screenshot_name("%AppName%\\%Date%_%Time%_%TimeMS%"), // Use a timestamp down to the millisecond because users may request more than one screenshot per-second
...
// ok so technically we could just try to pack everything from the font path into an atlas at startup but since not many people will switch fonts terribly often lets just do it as needed
// honestly maybe this an be generic files hmm
bool reshade::runtime::switch_to_next_font(std::filesystem::path filter_path, bool reversed)
{
  ...
  if (const std::filesystem::file_type file_type =
    ...
    if (current_font_index == std::numeric_limits<size_t>::max()) {
      // Current preset was not in the filter path, so just use the first or
      // last file
      if (reversed)
...
// And automatically handles trailing or duplicated slashes
