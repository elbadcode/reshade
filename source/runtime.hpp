...
namespace reshade
{
    ...
    private:
        ...
        bool execute_screenshot_post_save_command(
                    const std::filesystem::path &screenshot_path,
                    unsigned int screenshot_count, std::string postfix = "");
        ...
        // Would rather do this generically but switching fonts will need to trigger a reload for ImGui to work.
       //  I'm sure there's a more ImGui way to do it but for now I'll mostly copy the preset implementation. will next do audio
        bool switch_to_next_font(std::filesystem::path filter_path, bool reversed = false);
